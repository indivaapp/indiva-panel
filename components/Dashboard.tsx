...
const Dashboard: React.FC<DashboardProps> = ({ setActiveView, pendingAffiliateCount = 0, pendingSocialContentCount = 0 }) => {
    ...
    return (
        <div className="space-y-4 max-w-2xl">
            ...
            {/* Ürün Değerlendirme — mobilde BottomNav'da yer olmadığı için
                buradan erişilebiliyor (bkz. Sidebar'daki "productFeedback"). */}
            <button
                onClick={() => setActiveView('productFeedback')}
                className="w-full relative overflow-hidden rounded-2xl border border-emerald-500/20 text-left transition-all active:scale-[0.99]"
                style={{
                    background: 'linear-gradient(135deg, rgba(16,185,129,0.14) 0%, rgba(5,150,105,0.06) 100%)',
                    boxShadow: '0 0 0 1px rgba(16,185,129,0.1), 0 4px 24px rgba(0,0,0,0.25)',
                }}
            >
                <div className="absolute -right-4 -top-4 w-20 h-20 rounded-full bg-emerald-500/10 blur-xl pointer-events-none" />
                <div className="absolute -right-2 top-1/2 -translate-y-1/2 text-5xl opacity-10 pointer-events-none select-none">👍</div>

                <div className="relative px-4 py-4 flex items-center gap-4">
                    <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-xl shadow-lg shadow-emerald-900/30 shrink-0">
                        👍
                    </div>
                    <div className="flex-1">
                        <p className="text-white font-bold text-sm">Ürün Değerlendirme</p>
                        <p className="text-emerald-300/70 text-xs mt-0.5">Yayınlanan ürünleri beğen/beğenme — AI eğitim verisi</p>
                    </div>
                    <svg className="w-5 h-5 text-gray-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
