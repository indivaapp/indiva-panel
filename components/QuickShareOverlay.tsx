...
// 14 farklı palet — her story'de rastgele biri seçilir
const BG_PALETTES: [string, string, string][] = [
    ['#1a0800', '#c1440e', '#FF7A1A'], // koyu kırmızı-turuncu
    ['#3a1454', '#c2287a', '#ff7a1a'], // mor-pembe-turuncu
    ...
    ['#241005', '#9a3412', '#fbbf24'], // koyu kahve-turuncu-altın
    ['#170a12', '#7a1f3d', '#ff8c42'], // koyu bordo-kızıl-turuncu
];
...
// Açıklama metnini canvas'a çizer — URL satırlarını altın renkte gösterir.
// Döndürülen değer: çizilen kutunun yüksekliği (bir sonraki elemanın Y'si için).
...
async function renderStoryImage(
    ...
    // Paylaşılan tam ekran görüntüsünü yükle
    ...
    // Tam ekran görüntüsünü karta yerleştir (cover fit + bulanık dolgu)
    if (screenshotImg) {
        ctx.save();
        ...
        // Görseli karta sığdır (contain fit)
        const pad = 32;
    ...
    if (descriptionText.trim()) {
        ...
        drawDescriptionBox(ctx, descriptionText, 60, nextY, boxW, availH);
    } else {
        // Açıklama yoksa fiyat + tasarruf rozeti göster
    ...
    const footerLines = ['İNDİVA — Online Alışverişte İndirim & Fırsat Uygulaması'];
    let fy = ctaY + ctaH + 48;
    footerLines.forEach(line => { ctx.fillText(line, CANVAS_W / 2, fy); fy += 34; });
